...
void script::script_exception_handler(PEXCEPTION_POINTERS exp)
{
	...
	if (GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, (LPCWSTR)exp->ExceptionRecord->ExceptionAddress, &mod) == TRUE && mod != nullptr)
	{
		offset = ((DWORD64)exp->ExceptionRecord->ExceptionAddress - (DWORD64)mod);
		GetModuleFileNameA(mod, buffer, MAX_PATH - 1);
	}
	std::cout << "Exception Code: " << std::hex << exp->ExceptionRecord->ExceptionCode << " Exception Offset: " << offset << " Fault Module Name: " << buffer;
}
